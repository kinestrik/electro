...
export default {
  ...
  methods: {
    async login() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          this.userStore.fetchAdmin();
          await this.ActivateAdmin(); // Исправлен регистр
          this.$router.push("/");
        } else {
    ...
    async ActivateAdmin() { // Исправлен регистр
      try {
        const response = await axios.get("/user/activate-admin");
      } catch (e) {
        console.error(e);
      }
    },
    ...
    <div class="container mt-7">
      <div class="f">
        <div class="div-nickname">
          <h1 class="text-3xl text-bold registr mt-7 mb-7">Вход</h1>
          ...
          <div class="w-100 h-7">
            <a href="/sign-up" class="haveAcc">Нет аккаунта?</a>
          </div>
          <div class="w-100 h-7">
            <a href="/enter-email" class="haveAcc">Забыли пароль?</a>
          </div>
