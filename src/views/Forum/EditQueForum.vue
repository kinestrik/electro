...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        const res = await axios.put("/forum/update-q", {
          title: this.form.title,
          content: this.form.content,
          id_q: this.$route.params.id,
        });
        console.log("submitForm res", res); // Добавлено для отладки
        this.$router.push("/forum-que");
      } catch (e) {
        console.error(e);
        this.error = "Ошибка изменения вопроса.";
      }
    },
    async getQue() {
      try {
        ...
        console.log("getQue res", res); // Добавлено для отладки
      } catch (e) {
        console.error(e);
  ...
  mounted() {
    this.getQue();
    document.title = "Форум | Изменить вопрос";
  },
};
</script>
