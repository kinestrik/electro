...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        const res = await axios.post("/forum/add-s/text", {
          title: this.form.title, // <-- Использовать this.form.title
          content: this.form.content, // <-- Использовать this.form.content
        });
        this.$router.push("/forum-state")

      } catch (e) {
        console.error("Ошибка при отправке запроса:", e);
        this.error = "Ошибка создания статьи.";
      }
    },
  },
  components: {
  ...
  mounted() {
    document.title = "Форум | Создать статью";
  }
};
</script>
