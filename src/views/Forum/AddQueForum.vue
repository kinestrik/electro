...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        const response = await axios.post("/forum/add-q", {
        ...
        console.error(e);
        if (e.response && e.response.data.message === "Not in session") {
          this.error = "Войдите или зарегистрируйтесь.";
        } else {
          this.error = "Ошибка создания вопроса.";
        }
      }
    },
  },
  mounted() {
    document.title = "Форум | Задать вопрос";
  }
};
</script>
